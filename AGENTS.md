...
**依赖关系**：
- 依赖 RimMind-Core 提供的 API（`RimMindAPI`、`StructuredPromptBuilder`、`PromptBudget`、`ContextComposer`、`SettingsUIHelper`）和上下文构建
...
4. `_pendingPawns.Contains(pawnId)` — 同一小人是否正在请求
5. `RimMindAPI.ShouldSkipDialogue(pawn, type.ToString())` — Core 层跳过判定
6. `_pendingDialoguePairs.Contains(pairKey)` — 同一对话对是否正在请求（仅对话）
...
2. 反序列化 `AutoDialogueResponse`
3. `DisplayInteraction()` — MoteMaker.ThrowText 显示气泡
4. `ThoughtInjector.Inject()` — 注入独白 Thought
5. `ThoughtInjector.InjectRelationDelta()` — 注入关系 Thought（仅对话且有 relation_delta）
6. `AddLogEntry()` — 记录日志
...
- 使用 `StructuredPromptBuilder.FromKeyPrefix("RimMind.Dialogue.Prompt.System")`
- 调用 `.Goal()` 根据 recipient 是否为 null 选择 GoalMonologue / GoalDialogue
- 调用 `.Constraint()` 追加 TriggerReason、RelationDelta（仅对话）、角色约束、RecipientRole
- 调用 `.WithCustom(customPrompt)` 追加自定义 Prompt

**BuildPlayerSystemPrompt**：
- 使用 `StructuredPromptBuilder.FromKeyPrefix("RimMind.Dialogue.Prompt.PlayerSystem")`
- 调用 `.Role()` 设置角色名
- 调用 `.Constraint()` 追加 InitiatorConstraint + 发起者角色约束
- 调用 `.WithCustom(customPrompt)`

**BuildAutoUserPrompt**：
- 使用 `RimMindAPI.BuildFullPawnSections(pawn)` 构建 Pawn 上下文
- 根据触发类型追加翻译键上下文（Chitchat/Hediff/LevelUp/Thought/Auto）
- 对话时追加对话历史（使用 `ContextComposer.CompressHistory` 压缩）
- 使用 `PromptBudget(4000, 400)` 组装

**BuildPlayerUserPrompt**：
- 构建 Pawn 上下文 + 发起者上下文（tag 前缀 `initiator_`，优先级 -50）
- 使用 `PromptBudget(4000, 400)` 组装
...
static class DialogueService
{
    ...
    // AIRequest.MaxTokens = 300, Temperature = 0.85f
    // 响应中注入 Thought（recipient 为 null），记录 PlayerDialogueLog
}
```
...
调用方在写入前额外检查 `ModsConfig.IsActive("mcocdaa.RimMindMemory")`，避免反射开销。
...
由 `StructuredPromptBuilder.FromKeyPrefix` 自动拼接翻译键段落：
...
代码中额外动态追加的约束段落：
- `.TriggerReason` — 触发原因描述（通过 `builder.Constraint()` 追加）
- `.RelationDelta` — relation_delta 使用说明（仅对话时追加）
- 角色约束 — 发起者角色约束（通过 `GetRoleConstraint` 追加）
- `.RecipientRole` — 对话对象角色约束（Prisoner/Slave/Enemy/Visitor）
- `.WithCustom(customPrompt)` — 自定义 Prompt（追加在末尾）
...
| 段落 | 翻译键 | 内容 |
|------|--------|------|
| Role | `.Role` | 角色定义：扮演殖民者 {name}（代码通过 `.Role()` 动态设置） |
| Goal | `.Goal` | 目标：回应玩家对话 |
| Process | `.Process` | 流程 |
...
代码中额外动态追加的约束段落：
- 发起者角色约束（通过 `GetRoleConstraint(initiator)` 追加）
- `.WithCustom(customPrompt)` — 自定义 Prompt
...
| FloatMenuPatch | `FloatMenuMakerMap.ChoicesAtFor`(V1_5) / `.GetOptions`(V1_6) | 右键点击附近人形 Pawn | isImmediate: true，条件编译；使用 `RimMindAPI.ShouldSkipFloatMenu()` |
...
游戏事件（Chitchat/Hediff/LevelUp/Thought）
    ...
    │       ▼
    ├── RimMindAPI.RequestAsync()（自动对话）/ RequestImmediate()（玩家对话/右键菜单）
    │       ▼
    ├── AI 生成回复
    │       ▼
    ...
    │       ▼
    ├── MemoryBridge.AddMemory()（仅对话且 ModsConfig.IsActive("mcocdaa.RimMindMemory")）
    │       ▼
    ├── RecordDailyDialogue()
    │       ▼
...
| DialogueTrigger | 回调 | 注册外部对话触发回调（使用 `DialogueTriggerType.Chitchat` + `isImmediate: true`） |

## UI 层

### Window_Dialogue

聊天窗口，初始大小 480×540。构造函数 `Window_Dialogue(Pawn pawn, Pawn? initiator = null)`。支持滚动消息列表、输入框、发送按钮。消息按 User/Pawn 分色显示。等待 AI 回复时显示"思考中"状态。
...
6. **在 RimMindDialogueService.GetTriggerLabel 添加标签映射**
7. **调用 HandleTrigger**
...
2. **同步修改 RimMindDialogueService.GetRoleConstraint**（两处逻辑需保持一致）
3. **在翻译键 XML 添加 `RimMind.Dialogue.Prompt.Role.{Role}` 条目**
...
1. **线程安全**：所有游戏 API 调用在主线程执行，AI 回调在 `RimMindAPI.RequestAsync` / `RequestImmediate` 的回调中直接处理
...
10. **FloatMenuPatch**：使用 `RimMindAPI.ShouldSkipFloatMenu()` 控制右键菜单是否显示对话选项
